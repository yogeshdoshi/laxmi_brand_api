...
class Uploadimages extends CI_Controller {
    ...
    public function saveproduct() {
        ...
        foreach ($files['name'] as $key => $image) {
            ...
            $uniqueId = time() . '-' . mt_rand();
            $fileName = "PR_" . $pdt_name . "_" . time() . '.' . $ext;
        ...
        foreach ($images as $k => $im) {
            $img[] = base_url() . ASSETS_PATH . 'product/' . $pdt_name . "/" . $im;
            $img_path = $img;
        }
        ...
        $var_actual_price_count = 0;
        if (isset($jsonArray['var_actual_price']) && !empty($jsonArray['var_actual_price']) && is_array($jsonArray['var_actual_price'])) {
            $var_actual_price_count = count($jsonArray['var_actual_price']);
        } else {
            $resp["message"] = "Varient actual price is required";
            return $this->responsedata(400, 'failed', $resp);
        }
        ...
        if (($var_is_active_count == $var_discount_price_count) && ($var_is_active_count == $var_type_count) && ($var_is_active_count == $var_actual_price_count)) {
            ...
            foreach ($jsonArray['var_type'] as $k => $v) {
                $varient_array[] = array(
                    ...
                    "var_actual_price" => isset($jsonArray['var_actual_price'][$k]) ? $jsonArray['var_actual_price'][$k] : 0,
                    'created_at' => CURRENT_DATETIME,
                );
                $i++;
            }
